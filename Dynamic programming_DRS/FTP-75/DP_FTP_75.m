...
function [X, C, I, out] = DP_FTP_75(inp,~)
% DRS_DP_FTP_75 Computes the resulting state-of-charge based on current state-
% of-charge, inputs and drive cycle demand.
% 
% Function Signature:
% [X, C, I, out] = DRS_DP_FTP_75(INP,PAR)
%
% Inputs:
% INP   = input structure
...
load_system('torque_splits')  % Load the simulink model 'torque_splits'
...
n=10001;            % Number of control inputs (grd.Nu{1})
...
if (size(u1,1)~=1)
    ...
    % Run simulation
    outputs = sim('torque_splits', 'SrcWorkspace','current');
